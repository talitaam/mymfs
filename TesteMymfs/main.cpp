...
void importarArquivo(string caminhoComando, string caminhoArquivoImport)
{
    ...
    if(end<50000)
    {
        ...
        cout << "Naoo foi possivel importar o arquivo. Arquivo config.txt está cheio Mymfs." << endl;
}
...
//Dar merge
    ...
    if(arqConfigExiste.good() && !caminhoArquivoExport.empty()&& !caminhoDiretorioExport.empty())
    {
        ...
        while (strcmp(nomeDiretorioEncontrado.c_str(), nomeDiretorioBuscado.c_str()) != 0 &&
                !arqConfigExiste.eof() && !nomeDiretorioEncontrado.empty() ); //ETIDADO TALITA
                ...
                !nomeDiretorioEncontrado.empty() && !qtdArquivosEncontrado.empty())
        {
            ...
            std::ifstream arquivoJaExiste(caminhoDiretorioExport); //ETIDADO TALITA
            if(arquivoJaExiste.good())
            {
                arquivoJaExiste.close();
                cout << "O arquivo a ser exportado ja existe na pasta destino (" << caminhoDiretorioExport << "), por favor indique outro destino." << endl; //ETIDADO TALITA
            }
            else
            {
                std::ofstream combined_file(caminhoDiretorioExport); //ETIDADO TALITA
                for(int i=0; i<numArquivos; i++)
                {
                    auto s = std::to_string(i);
...
void listAll(string caminhoComando)  //ETIDADO TALITA
{
//Dar merge
    std::ifstream arqConfigExiste(caminhoComando + "/config.txt");
    if(arqConfigExiste.good())
